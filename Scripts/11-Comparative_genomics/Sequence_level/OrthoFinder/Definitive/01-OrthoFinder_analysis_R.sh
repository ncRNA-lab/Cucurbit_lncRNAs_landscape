...
#SBATCH --job-name=OFRdef					# Job name.
#SBATCH --output=OrthoFinder_R_definitive.log			# Standard output and error log.
...
F="/storage/ncRNA/Projects/lncRNAs/Cucurbitaceae/Scripts/Pascual/08-comparative_genomics/Sequence_level/OrthoFinder/Definitive/Functions_R.sh"
...
WD1=$WD/08-comparative_genomics/Sequence_level/OrthoFinder/r/Definitive
if [ -d "$WD1" ]; then
	rm -r $WD1
fi
...
mkdir -p $WD/08-comparative_genomics/Sequence_level/OrthoFinder/r
mkdir -p $WD/08-comparative_genomics/Sequence_level/OrthoFinder/r/Definitive
