...
#SBATCH --job-name=OMNRP2					# Job name.
#SBATCH --output=OrthoMCL_NR_P2.log				# Standard output and error log.
...
F="/storage/ncRNA/Projects/lncRNAs/Cucurbitaceae/Scripts/Pascual/08-comparative_genomics/Sequence_level/OrthoMCL/Prueba_filters_identity_percentage_and_alignment_length/Functions_NR.sh"
Species_list="car cla cma cme cmo cpe csa lsi mch"
Species_list_by_dash="car-cla-cma-cme-cmo-cpe-csa-lsi-mch"
...
WD1=$WD/08-comparative_genomics/Sequence_level/OrthoMCL/nr/Prueba_filters_identity_percentage_and_alignment_length
if [ -d "$WD1" ]; then
	rm -r $WD1
fi
...
mkdir -p $WD/08-comparative_genomics/Sequence_level/OrthoMCL/nr/Prueba_filters_identity_percentage_and_alignment_length
...
### SELECTION
## For each confidence level, class code and specie, create a TXT file with the identifiers of each lncRNA and a FASTA file with the sequences of the lncRNAs. We have to modify lncRNA IDs by adding the specie. This is important so that after running OrthoMCL we can distinguish which lncRNAs come from which species since all transcripts assembled with stringtie are called MSTRG regardless of the species.
cd $WD1/01-LncRNAs
...
### BLASTN
## For each confidence level and class code, run blastn for all possible pairwise combinations of the list of species including combinations with itself. Filenames can be TXT or TSV files.
cd $WD1/03-Blastn
...
### ORTHOMCL
## For each confidence level and class code, run OrthoMCL to create families of conserved lncRNAs using MCL clustering.
cd $WD1/04-OrthoMCL
...
for confidence in $Confidence_levels_list; do
	mkdir -p $confidence
	for class in $Classes_list; do
		srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive $F task_OrthoMCL $Species_list_by_dash $confidence $class $WD1 $WD $SP &
	done
done
wait

### FAMILIES
## For each confidence level and class code, get lncRNA families from OrthoMCL results.
cd $WD1/05-Families
