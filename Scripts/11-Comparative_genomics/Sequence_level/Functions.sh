...
task_Blastn(){
	...
	# are between different lncRNAs will be removed to avoid the presence of any 
	# paralog in the results.
...
task_OrthoFinder(){
	...
	orthofinder -f $DIR1 -d -o $DIR3/Prepare_blast -op -t $6 >> $DIR3/stdout_prepare_blast.log 2>&1
	...
	orthofinder -b $DIR3/Prepare_blast/$folder_p/WorkingDirectory/ -d -og -t $6 >> $DIR3/stdout_clustering.log 2>&1
	...
	if [ -d "Orthogroup_Sequences" ]; then
		rm -r "Orthogroup_Sequences"
	fi
	if [ -d "Single_Copy_Orthologue_Sequences" ]; then
