...
#SBATCH --job-name=ILPR							# Job name.
#SBATCH --output=Identify_LncRNAs_PAIRWISE_SPECIES_R.log			# Standard output and error log.
...
for j in `seq 1 $n_comb`; do
	...
	srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive $F task_identify_LncRNAs_inside_syntenic_blocks $spe1 $spe2 $WD1/03-Adhore/PAIRWISE_SPECIES $WD1/04-LncRNAs_inside_syntenic_blocks/PAIRWISE_SPECIES $WD/05-predict_lncRNAs 'r' $AS &
done
wait
