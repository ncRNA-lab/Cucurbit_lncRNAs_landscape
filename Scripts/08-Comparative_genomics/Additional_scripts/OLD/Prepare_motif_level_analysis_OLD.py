...
def CreateFastaFilesREAL (Families_dic, LncRNAs_dic, path_out):
    
    for fam, lncs in Families_dic.items():
        fam_fasta = open(path_out + "/real/" + fam + ".fasta", "w")
        for lnc in lncs:
            fam_fasta.write(">" + lnc + "\n" + LncRNAs_dic[lnc] + "\n")
        fam_fasta.close()
        
def CreateFastaFilesSIMULATIONS (iterations, Families_dic, LncRNAs_by_specie_dic, path_out):
    ...
    for i in iterations:
        ...
        for fam, lncs in Families_dic.items():
            ...
            for lnc in lncs:
                spe = lnc.split("-")[1]
                dic_spe = LncRNAs_by_specie_dic[spe]
                other_lncs = [k for k in list(dic_spe.keys()) if k != lnc]
                random_lnc = random.choice(other_lncs)
...
def main():
    ...
    ## Create a dictionary with the LncRNA IDs as a key and its sequence as value.
    LncRNAs_dic = LncRNAsDict (fasta_in)
    
    ## Create a dictionary with the LncRNA IDs as a key and its sequence as value
    ## taking into account the specie.
    LncRNAs_by_specie_dic = LncRNAsDictBySpecie (fasta_in)
    
    ## REAL: Create a fasta file by conserved family.
    print("\n-REAL")
    os.makedirs(path_out + "/real")
    CreateFastaFilesREAL (Families_dic, LncRNAs_dic, path_out)
    ...
    for i in range(n_proc):
        if i == n_proc-1:
            ...
            pf = pi + dist
        processes.append(multiprocessing.Process(target = CreateFastaFilesSIMULATIONS , args=(iterations[pi:pf], Families_dic, LncRNAs_by_specie_dic, path_out) )) 
        processes[i].start()
