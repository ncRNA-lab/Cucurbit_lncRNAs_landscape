...
#SBATCH --job-name=BNRP1					# Job name.
#SBATCH --output=Blastn_NR_P1.log				# Standard output and error log.
...
F="/storage/ncRNA/Projects/lncRNAs/Cucurbitaceae/Scripts/Pascual/08-comparative_genomics/Sequence_level/Blastn/Prueba_e-value/Functions_NR.sh"
...
WD1=$WD/08-comparative_genomics/Sequence_level/Blastn/nr/Prueba_e-value
if [ -d "$WD1" ]; then
	rm -r $WD1
fi
...
mkdir -p $WD/08-comparative_genomics/Sequence_level/Blastn/nr/Prueba_e-value
