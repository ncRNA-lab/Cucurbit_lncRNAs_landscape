		...
		for confidence in $Confidence_levels_list; do
			...
			for class in $Classes_list; do
				...
				for width in $widths; do
					...
					srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive $F task_lncLOOM_2 $DIR_A/real $DIR_B/real $SLURM_CPUS_PER_TASK $width $DIR_B/outputs/stdout_REAL.log &
					...
					for i in $(seq $iterations); do
						...
						srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive $F task_lncLOOM_2 $DIR_A/simulations/iter_$i $DIR_B/simulations/iter_$i $SLURM_CPUS_PER_TASK $width $DIR_B/outputs/stdout_SIMULATION_$i.log &
