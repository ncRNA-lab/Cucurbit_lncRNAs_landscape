...
#SBATCH --job-name=PosNR3M							# Job name.
#SBATCH --output=Positional_NR_3_MEME.log					# Standard output and error log.
				...
				for mode in $modes; do
					...
					for width in $widths; do
						...
						for fam in $families; do 
							srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive $F task_MEME_1 $fam $DIR_A/real $DIR_B/real $mode $width &
						done
						wait
						...
						for i in $(seq $iterations); do
							...
							for fam in $families; do 
								srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive $F task_MEME_1 $fam $DIR_A/simulations/iter_$i $DIR_B/simulations/iter_$i $mode $width &
							done
							wait
						done
