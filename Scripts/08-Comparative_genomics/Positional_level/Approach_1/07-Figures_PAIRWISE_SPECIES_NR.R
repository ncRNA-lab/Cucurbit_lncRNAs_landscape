...
WD1 = "/mnt/doctorado/3-lncRNAs/Cucurbitaceae/Results/08-comparative_genomics/Synteny/nr/05-Families/PAIRWISE_SPECIES"
WD2 = "/mnt/doctorado/3-lncRNAs/Cucurbitaceae/Results/08-comparative_genomics/Synteny/nr/06-Figures"
...
WD2 = paste0(WD2, "/PAIRWISE_SPECIES")
if (!dir.exists(WD2)){
  dir.create(WD2)
}
...
for (confidence in confidences) {
  for (class in classes) {
    ...
    # It's important to check if the file exists. In OrthoMCL if there are some 
    # blast table without hits fail and then doesn't generate the all_orthomcl.out 
    # table. This table is necessary
...
for (confidence in confidences) {
  for (class in classes) {
    if (file.exists(paste0(WD1, "/", confidence, "/", class, "/gen.tsv"))) {
      ...
      fam = gen[,c("Family", "Type", "Number_species_by_family", "Specie")]
      ...
      fam_red_1 = fam %>% 
        ...
        summarise(
          sum.Counts = sum(Counts)) %>%
        mutate(Total = sum(sum.Counts),
               perc = round(sum.Counts/sum(sum.Counts) * 100, 2))
      ...
      colnames(fam_red_2) = c("Confidence", "Class", "Specie", "Type", "Counts_Families", "Total_Families", "Percentage_Families")
...
ggsave(paste0(WD2, "/Stacked_barplot_Families_1.png"), height = 18, width = 14, dpi = 600)
ggsave(paste0(WD2, "/Stacked_barplot_Families_1.pdf"), height = 18, width = 14, dpi = 600)

TAB_2_FAM$Confidence = factor(TAB_2_FAM$Confidence, levels = confidences)
TAB_2_FAM$Class = factor(TAB_2_FAM$Class, levels = classes)

gg2 = ggplot(TAB_2_FAM, aes(x = Confidence, y = Percentage_Families, fill = Type)) +
  geom_bar(colour = "black", position="fill", stat="identity") +
  facet_grid(Class ~ Specie) +
...
ggsave(paste0(WD2, "/Stacked_barplot_Families_2.png"), height = 18, width = 14, dpi = 600)
ggsave(paste0(WD2, "/Stacked_barplot_Families_2.pdf"), height = 18, width = 14, dpi = 600)
...
for (confidence in confidences) {
  for (class in classes) {
    if (file.exists(paste0(WD1, "/", confidence, "/", class, "/gen.tsv"))) {
      ...
      gen$"Type" = ifelse(gen$Number_species_by_family > 1, "Conserved", "Non-conserved")
      gen$"Specie" = sapply(strsplit(gen$Member, "-"), "[[", 2)
      gen = gen[,c("Member", "Type", "Number_species_by_family", "Specie")]
      ...
      colnames(gen_red_2) = c("Confidence", "Class", "Specie", "Type", "Counts_LncRNAs", "Total_LncRNAs", "Percentage_LncRNAs")
...
gg2 = ggplot(TAB_2_LNCRNAS, aes(x = Confidence, y = Percentage_LncRNAs, fill = Type)) +
  geom_bar(colour = "black", position="fill", stat="identity") +
  facet_grid(Class ~ Specie) +
  scale_fill_manual(values = c("#bc3c29", "#0072b5")) +
  scale_y_continuous(expand = c(0.02, 0.05)) +
...
ggsave(paste0(WD2, "/Stacked_barplot_LncRNAs_2.png"), height = 18, width = 14, dpi = 600)
ggsave(paste0(WD2, "/Stacked_barplot_LncRNAs_2.pdf"), height = 18, width = 14, dpi = 600)
...
for (confidence in confidences) {
  for (class in classes) {
    if (file.exists(paste0(WD1, "/", confidence, "/", class, "/gen.tsv"))) {
      ...
      gen$"Number_species_by_family" = rowSums(gen[,species])
      gen$"Type" = ifelse(gen$Number_species_by_family > 1, "Conserved", "Non-conserved")
      gen = gen[, c("Family", "Member", "LncRNA", "Specie", "Number_species_by_family", "Type")]
      gen$"Confidence" = confidence
      gen$"Class" = class
      for (spe in species) {
        TPMs = read.table(paste0(WD3, "/", spe, "/Salmon/ALL/nr/04-Table/TPMs_summary.tsv"), sep = "\t", header = T, quote = "\"")
...
gg2 = ggplot(TAB_1_LNCRNAS_TPMs, aes(x = Confidence, y = log2.TPMs.1.mean, fill = Type)) +
