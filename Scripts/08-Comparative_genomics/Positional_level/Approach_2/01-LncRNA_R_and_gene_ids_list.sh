...
#SBATCH --job-name=RP1						# Job name.
#SBATCH --output=R_P1.log					# Standard output and error log.
...
for confidence in $Confidence_levels_list; do
	...
	for class in $Classes_list; do
		...
		for spe in $Species_list; do
			srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive $F task_lncRNAS_R_and_genes_list $spe $confidence $class $WD $WD1 $AS &
		done
		wait
	done
done
