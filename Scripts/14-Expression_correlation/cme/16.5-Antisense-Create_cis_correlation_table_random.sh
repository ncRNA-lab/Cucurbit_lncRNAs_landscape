...
for flag in $flag_list; do
	...
	if [ -d "$WD3_spe/03-Metadata_DEA/" ] && [ "$(ls -A "$WD3_spe/03-Metadata_DEA/")" ]; then
		...
		for exp in $experiment_list; do
			...
			srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive --output $O5/Outputs/Closest-$exp.log $F task_STEP5_CLOSEST-ANTISENSE "$specie_long" $exp $O5 $WD1_spe $WD2_spe $WD3_spe 1000 $flag $AS &
		done
		wait
	else
