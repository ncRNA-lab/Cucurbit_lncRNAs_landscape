...
if [ -d "$WD1/02-Trimmed_data/Filter_table/Temp" ]; then
	cd $WD1/02-Trimmed_data/Filter_table
	rm -r Temp
	mkdir Temp
	cd $WD1/02-Trimmed_data
else
	mkdir $WD1/02-Trimmed_data/Filter_table/Temp
fi

# Remove temporal directory of accession_list directory.
if [ -d "$AI/sra-info/accession_list/Temp_1" ]; then
	cd $AI/sra-info/accession_list
	rm -r Temp_1
	mkdir Temp_1
	cd $WD1/02-Trimmed_data
else
	mkdir $AI/sra-info/accession_list/Temp_1
fi

# Calculate library depth and determine if samples pass the filter.
for SRR in $(cat $Acc_list); do
	srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --output $WD1/02-Trimmed_data/outputs/stdout_Summary_table_trimming_$SRR.log --quiet --exclusive $F task_Summary_table_trimming $SRR $WD1/02-Trimmed_data/Filter_table/Temp $AI/sra-info/accession_list/Temp_1 &
done
wait
...
cd $AI/sra-info/accession_list
find Temp_1 -type f -name '*[0-9].txt' -print0 | xargs -0 cat > $New_Acc_list
rm -r Temp_1
