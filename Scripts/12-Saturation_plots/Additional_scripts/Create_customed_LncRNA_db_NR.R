...
#EXAMPLE --> Create_customed_LncRNA_db_NR.R $WD2/04-Predict_lncRNAs "Batch-1"
...
DB = read.table(paste0(WD, "/STEP-FINAL/", Batch, "/Database/Database_LncRNAs.tsv"), sep = "\t", header = T, quote = "\"")

### Load results of redundancy analysis
AGAT = read.table(paste0(WD, "/STEP-FINAL/", Batch, "/Redundancy_analysis/AGAT/POTENTIAL_LNCRNAS_filt_sort_ids.txt"), header = F, quote = "\"")
AGAT = AGAT$V1
CGAT = read.table(paste0(WD, "/STEP-FINAL/", Batch, "/Redundancy_analysis/CGAT/POTENTIAL_LNCRNAS_sort_filt_sort_ids.txt"), header = F, quote = "\"")
CGAT = CGAT$V1
CDHIT90 = read.table(paste0(WD, "/STEP-FINAL/", Batch, "/Redundancy_analysis/CDHIT/POTENTIAL_LNCRNAS_filt_90_ids.txt"), header = F, quote = "\"")
CDHIT90 = CDHIT90$V1
CDHIT95 = read.table(paste0(WD, "/STEP-FINAL/", Batch, "/Redundancy_analysis/CDHIT/POTENTIAL_LNCRNAS_filt_95_ids.txt"), header = F, quote = "\"")
CDHIT95 = CDHIT95$V1
...
write.table(DB, paste0(WD, "/STEP-FINAL/", Batch, "/Database/Database_LncRNAs.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)

### Select non-redundant lncRNAs.
DB_nr = DB[DB$CGAT == "non-redundant",]
write.table(DB_nr, paste0(WD, "/STEP-FINAL/", Batch, "/Database/Database_LncRNAs_NR.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)


## 3. FIGURES

if (!dir.exists(paste0(WD, "/STEP-FINAL/", Batch, "/Redundancy_analysis/Figures"))){
  dir.create(paste0(WD, "/STEP-FINAL/", Batch, "/Redundancy_analysis/Figures"))
}
...
png(filename=paste0(WD, '/STEP-FINAL/', Batch, '/Redundancy_analysis/Figures/LncRNAs_R_and_NR_prediction_tools.png'), height = 5000, width = 5000, res=600)
vennDiagram(Z, 
            include="both", 
            names=NULL, 
            mar=rep(1,4), 
...
png(filename=paste0(WD, '/STEP-FINAL/', Batch, '/Redundancy_analysis/Figures/LncRNAs_NR_prediction_tools.png'), height = 5000, width = 5000, res=600)
vennDiagram(Z, 
            include="both", 
            names=NULL, 
            mar=rep(1,4), 
...
png(filename = paste0(WD, '/STEP-FINAL/', Batch, '/Redundancy_analysis/Figures/LncRNAs_NR_redundancy_tools_4.png'), width = 3000, height = 3000, res = 500)
ggvenn(
  x, 
...
png(filename = paste0(WD, '/STEP-FINAL/', Batch, '/Redundancy_analysis/Figures/LncRNAs_NR_redundancy_tools_2.png'), width = 3000, height = 3000, res = 500)
ggvenn(
  x, 
...
ggsave(paste0(WD, "/STEP-FINAL/", Batch, "/Redundancy_analysis/Figures/Class_code_lncRNAs_distribution_NR.png"), height = 7, width = 8, dpi = 600)
