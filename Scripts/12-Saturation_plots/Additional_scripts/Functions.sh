...
task_Merge_assemblies(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	Pmer=$3/02-Merged_assemblies
	
	####### PIPELINE
	cd $Pmer
	
	## Create a TXT file with the assembly (GTF file) route for each sample.
	awk -v V1=$2 '{ print V1"/01-Individual_assembly/"$1".gtf" }' $3/01-Batches/Batch-$1.txt > assemblies-$1.txt
...
task_Annotate_transcripts(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	Pmer=$2/02-Merged_assemblies
	Pann=$2/03-Annotate_transcripts
	
	####### PIPELINE
	cd $Pann
	
	## Annotate transcripts.
	gffcompare $Pmer/$4\_merged-$1.gtf -o $4\_merged_compared-$1 -r $3/GTF_genes/$4.gtf -V
	mv $Pmer/$4\_merged_compared-$1.$4\_merged-$1.gtf.tmap $4\_merged_compared-$1.$4\_merged-$1.gtf.tmap
	mv $Pmer/$4\_merged_compared-$1.$4\_merged-$1.gtf.refmap $4\_merged_compared-$1.$4\_merged-$1.gtf.refmap
	tail -n +2 $4\_merged_compared-$1.$4\_merged-$1.gtf.tmap | awk '{print $3}' | sort | uniq -c
}

task_Predict_lncRNAs_STEP1(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	P1=$2/04-Predict_lncRNAs/STEP1/Batch-$1
	
	####### DIRECTORY
	mkdir -p $P1
	mkdir -p $P1/Original_genes
	mkdir -p $P1/Potential_lncRNAs
	...
	GFF3orGTF2TABLE.py \
		--gtf $2/03-Annotate_transcripts/$4\_merged_compared-$1.annotated.gtf \
	...
	echo -e "\nGet GTF..."
	Filter_GTF.py \
		--gtf-initial $2/03-Annotate_transcripts/$4\_merged_compared-$1.annotated.gtf \
...
task_Predict_lncRNAs_STEP2(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	P1=$2/04-Predict_lncRNAs/STEP1/Batch-$1
	P2=$2/04-Predict_lncRNAs/STEP2/Batch-$1
	...
	FEELnc_codpot.pl --outdir="./FEELnc" -o output_FEELnc -i $P1/Potential_lncRNAs/POTENTIAL_LNCRNAS.fasta -a $P1/Original_genes/ORIGINAL_GENES.fasta --mode=shuffle --proc $4
	...
	cpat.py -g $P1/Potential_lncRNAs/POTENTIAL_LNCRNAS.fasta -o ./CPAT/output_cpat -x $3/cpat-3.0.2/crema_models/ath_hexamer.tsv -d $3/cpat-3.0.2/crema_models/ath_logit.RData
...
task_Predict_lncRNAs_STEP3(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	P1=$2/04-Predict_lncRNAs/STEP1/Batch-$1
	P3=$2/04-Predict_lncRNAs/STEP3/Batch-$1
	...
	diamond makedb --in $3/Swissprot/uniprot_sprot.fasta -d ./SwissProt/swissprot.dmnd
	...
	Add_info_to_Swissprot_blastx_tsv.py \
		...
		--fasta $3/Swissprot/uniprot_sprot.fasta \
	...
	cp $3/PFAM/Pfam-A.hmm ./Pfam/Hmmer/
...
task_Predict_lncRNAs_STEP4(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	P1=$2/04-Predict_lncRNAs/STEP1/Batch-$1
	P4=$2/04-Predict_lncRNAs/STEP4/Batch-$1
...
task_Predict_lncRNAs_STEP5(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	P1=$2/04-Predict_lncRNAs/STEP1/Batch-$1
	P5=$2/04-Predict_lncRNAs/STEP5/Batch-$1
	...
	makeblastdb \
		-in $3/RNAcentral/ALL/cucurbitaceae_RNAcentral.fasta \
		-dbtype nucl \
		-parse_seqids \
	...
	Add_info_to_RNAcentral_blastn_tsv.py \
		...
		--fasta $3/RNAcentral/ALL/cucurbitaceae_RNAcentral.fasta \
	...
	echo -e "Making the Blast database..."
	makeblastdb \
		-in $3/miRNAs/PmiREN/ALL/cucurbitaceae_miRNA_precursors.fa \
		-dbtype nucl \
		-parse_seqids \
	...
	echo -e "Making the Blast database..."
	makeblastdb \
		-in $3/miRNAs/PmiREN/ALL/cucurbitaceae_miRNA_mature.fa \
		-dbtype nucl \
		-parse_seqids \
	...
	echo -e "Making the Blast database..."
	makeblastdb \
		-in $3/miRNAs/miRBase/ALL/cucurbitaceae_miRNA_precursors.fa \
		-dbtype nucl \
		-parse_seqids \
	...
	echo -e "Making the Blast database..."
	makeblastdb \
		-in $3/miRNAs/miRBase/ALL/cucurbitaceae_miRNA_mature.fa \
		-dbtype nucl \
		-parse_seqids \
...
task_Predict_lncRNAs_STEP6(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	P1=$2/04-Predict_lncRNAs/STEP1/Batch-$1
	P6=$2/04-Predict_lncRNAs/STEP6/Batch-$1
	...
	echo -e "Making the Blast database..."
	makeblastdb \
		-in $3/LncRNAs_databases/CANTATAdb/ALL/cucurbitaceae_CANTATAdb.fa \
		-dbtype nucl \
		-parse_seqids \
	...
	echo -e "Making the Blast database..."
	makeblastdb \
		-in $3/LncRNAs_databases/PLncDB/ALL/cucurbitaceae_PLncDB.fa \
		-dbtype nucl \
		-parse_seqids \
		-out ./PLncDB/database \
		-title LncRNAs
	...
	echo -e "Making the Blast database..."
	makeblastdb \
		-in $3/LncRNAs_databases/GreeNC/ALL/cucurbitaceae_GreeNC.fa \
		-dbtype nucl \
		-parse_seqids \
		-out ./GreeNC/database \
		-title LncRNAs
...
task_Predict_lncRNAs_STEP-FINAL(){
	echo -e "\tBatch "$1"..."
	
	####### VARIABLES
	P1=$2/04-Predict_lncRNAs/STEP1/Batch-$1
	P7=$2/04-Predict_lncRNAs/STEP-FINAL/Batch-$1
	...
	echo -e "\nCREATE THE LNCRNAS DATABASE...\n"
	Rscript $4/Create_customed_LncRNA_db.R $6 $2/04-Predict_lncRNAs Batch-$1 $3 $5
	...
	## fasta
	echo -e "\nCreate fasta file..."
	gffread -w ALL_temp.fasta -W -g $3/Genome/$6.fa ALL.gtf
	...
	## fasta
	echo -e "\nCreate fasta file..."
	gffread -w FILTERED_temp.fasta -W -g $3/Genome/$6.fa FILTERED.gtf
	...
	echo -e "\nCreate tsv file..."
	cp $FL/Genes/ORIGINAL_GENES.tsv ./
	gffread -w PL_temp.fasta -W -g $3/Genome/$6.fa POTENTIAL_LNCRNAS_pred_x_u_filtered.gtf
	...
	cd-hit-est -i $FL/LncRNAs/r/POTENTIAL_LNCRNAS_pred.fasta -o ./CDHIT/POTENTIAL_LNCRNAS_filt_90.fasta -c 0.90 -n 9 -d 999 -M 0 -T $7 1> ./CDHIT/stdout_90.txt
	...
	cd-hit-est -i $FL/LncRNAs/r/POTENTIAL_LNCRNAS_pred.fasta -o ./CDHIT/POTENTIAL_LNCRNAS_filt_95.fasta -c 0.95 -n 10 -d 999 -M 0 -T $7 1> ./CDHIT/stdout_95.txt
	...
	gffread -w ./AGAT/POTENTIAL_LNCRNAS_filt_sort_temp.fasta -W -g $3/Genome/$6.fa ./AGAT/POTENTIAL_LNCRNAS_filt_sort.gtf
	...
	gffread -w ./CGAT/POTENTIAL_LNCRNAS_sort_filt_sort_temp.fasta -W -g $3/Genome/$6.fa ./CGAT/POTENTIAL_LNCRNAS_sort_filt_sort.gtf
	...
	echo -e "\nCREATE THE NON-REDUNDANT LNCRNAS DATABASE...\n"
	Rscript $4/Create_customed_LncRNA_db_NR.R $2/04-Predict_lncRNAs Batch-$1
