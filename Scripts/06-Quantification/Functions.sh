...
task_Salmon_All(){
	...
	if [ -e $2/04-Selected_data/$1"_tr_1P.fastq.gz" ] && [ -e $2/04-Selected_data/$1"_tr_2P.fastq.gz" ]; then
		if [ $LIBTYPE = "ISR" ] || [ $LIBTYPE = "ISF" ]; then
			...
			salmon quant \
				-i $4/ALL/$7/02-Index/$8 \
				-l $LIBTYPE \
				-1 $2/04-Selected_data/$1"_tr_1P.fastq.gz" \
				-2 $2/04-Selected_data/$1"_tr_2P.fastq.gz" \
				-g $3/STEP-FINAL/Files/Joined/ALL/$7/ALL.gtf \
				-p $6 \
				-o $4/ALL/$7/03-Quant/$1 \
				--gcBias
		else
...
task_Salmon_LncRNAs(){
	...
	if [ -e $2/04-Selected_data/$1"_tr_1P.fastq.gz" ] && [ -e $2/04-Selected_data/$1"_tr_2P.fastq.gz" ]; then
		if [ $LIBTYPE = "ISR" ] || [ $LIBTYPE = "ISF" ]; then
			echo -e $1" is a strand-specific library (ISR or ISF)."
			salmon quant \
				-i $4/LNCRNAS/$7/02-Index/$8 \
				-l $LIBTYPE \
				-1 $2/04-Selected_data/$1"_tr_1P.fastq.gz" \
				-2 $2/04-Selected_data/$1"_tr_2P.fastq.gz" \
				-g $3/STEP-FINAL/Files/LncRNAs/$7/POTENTIAL_LNCRNAS_pred.gtf \
				-p $6 \
				-o $4/LNCRNAS/$7/03-Quant/$1 \
				--gcBias
		else
			echo -e $1" is unstranded library (IU)."
			salmon quant \
