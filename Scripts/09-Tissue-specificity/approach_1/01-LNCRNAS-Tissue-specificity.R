################################################################################
#
# LNCRNAS: TISSUE SPECIFICITY STUDY: APPROACH 1 - STEP 1
#
...
for (spe in species) {
  ...
  # Transcripts info.
  gtf_file = paste0(path_pred, "/", spe, "/STEP-FINAL/Files/LncRNAs/", flag, "/POTENTIAL_LNCRNAS_pred.gtf")
  ...
  quant_dir = paste0(path_quant, "/", spe, "/Salmon/LNCRNAS/", flag, "/03-Quant/")
  ...
  # Create tab table.
  cat(paste0("\n\nCreating tab table\n"))
  TPMs = merge(TPMs, LncRNAs_info, by = "ID_transcript", all = T)
  ...
  if (length(SRA.studies) > 0) {
    for (SRA.study in SRA.studies) {
      ...
      write.table(tab_SRA_mean_wide, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/", spe, "/LNCRNAS-", SRA.study, "_mean.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
      write.table(tab_SRA_median_wide, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/", spe, "/LNCRNAS-", SRA.study, "_median.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
    }
...
# Save tables.
write.table(TAB, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/LNCRNAS-TAB_TPMs.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(TAB_red, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/LNCRNAS-TAB_TPMs_summary.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(TAB_red_filt, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/LNCRNAS-TAB_TPMs_summary_filtered.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(TAB_experiments, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/LNCRNAS-TAB_experiments.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(TAB_experiments_summary, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/LNCRNAS-TAB_experiemnts_summary.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
