################################################################################
#
# GENES: TISSUE SPECIFICITY STUDY: APPROACH 1 - STEP 1
#
...
if (!dir.exists(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL"))){
  dir.create(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL"))
}
if (!dir.exists(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag))){
  dir.create(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag))
}
if (!dir.exists(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1"))){
  dir.create(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1"))
}
...
for (spe in species) {
  ...
  # Transcripts info.
  gtf_file = paste0(path_pred, "/", spe, "/STEP-FINAL/Files/Genes/ORIGINAL_GENES.gtf")
  ...
  quant_dir = paste0(path_quant, "/", spe, "/Salmon/GENES/03-Quant/")
  ...
  # Create tab table.
  cat(paste0("\n\nCreating tab table\n"))
  TPMs = merge(TPMs, Genes_info, by = "ID_transcript", all = T)
  ...
  if (!dir.exists(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/", spe))){
    dir.create(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/", spe))
  }
  ...
  if (length(SRA.studies) > 0) {
    for (SRA.study in SRA.studies) {
      ...
      write.table(tab_SRA_mean_wide, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/", spe, "/GENES-", SRA.study, "_mean.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
      write.table(tab_SRA_median_wide, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/", spe, "/GENES-", SRA.study, "_median.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
    }
    
    rm(list = c("tab_SRA_mean_long", "tab_SRA_median_long", "tab_SRA_mean_wide", "tab_SRA_median_wide"))
  }
  
  rm(list = c("gtf_file", "txdb", "k", "df", "tx2gene", "metadata_file", "files", "txi.salmon", "Genes_info", 
...
# Save tables.
write.table(TAB, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/GENES-TAB_TPMs.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(TAB_red, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/GENES-TAB_TPMs_summary.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(TAB_red_filt, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/GENES-TAB_TPMs_summary_filtered.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(TAB_experiments, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/GENES-TAB_experiments.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(TAB_experiments_summary, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP1/GENES-TAB_experiemnts_summary.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
