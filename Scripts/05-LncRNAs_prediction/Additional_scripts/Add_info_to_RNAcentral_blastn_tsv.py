...
"""
- ADD INFO TO TSV FILE COMING FROM RNACENTRAL BLASTN.

The sseqid column contained in the TSV file indicates the id of the RNAcentral's 
nucleotide sequence, but this is only an ID. Then, this script adds info to know 
what is this ID using the headers of the RNAcentral database fasta file which 
can be downloaded from https://rnacentral.org/. If the fasta file has not more 
info than the ID, this script will be useless.

---------

Created on Fri Jun 08 13:30:00 2022

Last Modified:
    - Fri Jun 08 13:30:00 2022 --> Initial code.
...
def TSVtoLIST (tsv):
    ...
    the TSV file coming from blastn using as a reference (source) 
...
def Write_table (tsv, records, info):
    ...
    f_out = open(tsv, 'w')
    f_out.write("qseqid\tqlen\tsseqid\tslen\tqstart\tqend\tsstart\tsend\tqframe\tsframe\tsstrand\tpident\tgaps\tlength\tevalue\tbitscore\tsinfo\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
            prog='AddinfotoRNACentralBLASTntsv V1', 
            description='''This program adds info to RNAcentral BLASTn TSV table results''', 
    ...
    parser.add_argument(
            ...
            type=str, 
            nargs=1,
            help="Absolute path of initial RNAcentral BLASTn table results."
            )
    parser.add_argument(
            "-d", 
            "--fasta-db", 
    ...
    parser.add_argument(
            ...
            type=str, 
            nargs=1, 
            help="Absolute path of final RNAcentral BLASTn table results."
            )
    parser.add_argument(
            '--version', 
            action='version', 
    ...
    # tsv1 =  "/storage/ncRNA/Projects/lncRNAs/Cucurbitaceae/Results/05-LncRNAs_prediction/cme/STEP5/RNAcentral/output_blastn_temp.tsv"
