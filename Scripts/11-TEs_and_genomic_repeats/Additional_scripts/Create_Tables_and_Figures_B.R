...
# be changed in the script) of some kind of transposon that can be found in the 
# genome of each specie. We take always into account the class code of each transcript.
#
################################################################################
...
#EXAMPLE --> Create_Tables_and_Figures_B.R path_res folder_new folder_RepMask flag confidence
...
for (spe in species) {
  ...
  if (!dir.exists(paste0(WD_01, "/Figures_and_Tables/B"))){
    dir.create(paste0(WD_01, "/Figures_and_Tables/B"))
  }
  ...
  TAB$"Repeat_type_2_mod" = ifelse(grepl("LINE", TAB$Repeat_type_2, fixed = T), "Transposon",
                                   ifelse(grepl("SINE", TAB$Repeat_type_2, fixed = T), "Transposon",
                                          ifelse(grepl("LTR", TAB$Repeat_type_2, fixed = T), "Transposon",
                                                 ifelse(grepl("DNA/", TAB$Repeat_type_2, fixed = T), "Transposon",
                                                        ifelse(grepl("RC/", TAB$Repeat_type_2, fixed = T), "Transposon",
                                                               ifelse(grepl("Retroposon", TAB$Repeat_type_2, fixed = T), "Transposon",
                                                                      ifelse(grepl("PLE/", TAB$Repeat_type_2, fixed = T), "Transposon",
                                                                             "No transposon")))))))
  ...
  TAB$Repeat_type_2_mod = factor(TAB$Repeat_type_2_mod, levels = c("Transposon"))
...
### Save TAB_FINAL.
write.table(TAB_FINAL, paste0(WD_01, "/Figures_and_Tables/B/Final_tab-", flag, "-", confidence, ".tsv"), col.names = T, row.names = F, sep = "\t", quote = F)
...
TAB_SUMMARY = TAB_SUMMARY[, c("spe", "Class_code", "Percentage")]
colnames(TAB_SUMMARY) = c("spe", "Class_code", "Transposon")

### Save TAB_SUMMARY.
write.table(TAB_SUMMARY, paste0(WD_01, "/Figures_and_Tables/B/Summary_tab-", flag, "-", confidence, ".tsv"), col.names = T, row.names = F, sep = "\t", quote = F)

### Remove variables.
rm(list = c("ALL", "All_genes", "All_IR", "All_lncRNAs", "LncRNAs_db", "RES", 
            "res_genes", "res_IR", "res_lncRNAs", "spe", "TAB", "WD_02", 
            "TAB_SUMMARY_individual", "TAB_SUMMARY_total", "masked_perc"))

### 2.3 FIGURE

cat("\nDrawing the TAB SUMMARY table...\n")

TAB_SUMMARY = merge(TAB_SUMMARY, species_tab, by = "spe", all.x = T, all.y = F)
TAB_SUMMARY$name = factor(TAB_SUMMARY$name, levels = species_name)

gg1 = ggplot(TAB_SUMMARY, aes(x = Class_code, y = Transposon, fill = Class_code)) +
  geom_bar(position=position_dodge(), aes(y=Transposon), colour="black", stat="identity") +
  ...
  xlab("") +
  ylab("Transcripts with transposon content (%)") +
  theme_bw() + 
...
ggsave(paste0(WD_01, "/Figures_and_Tables/B/Percentage_transposon-", flag, "-", confidence, ".png"), height = 5, width = 25, dpi = 600)
ggsave(paste0(WD_01, "/Figures_and_Tables/B/Percentage_transposon-", flag, "-", confidence, ".pdf"), height = 5, width = 25, dpi = 600)
...
ggsave(paste0(WD_01, "/Figures_and_Tables/B/Percentage_transposon_mod-", flag, "-", confidence, ".png"), height = 5, width = 25, dpi = 600)
ggsave(paste0(WD_01, "/Figures_and_Tables/B/Percentage_transposon_mod-", flag, "-", confidence, ".pdf"), height = 5, width = 25, dpi = 600)
