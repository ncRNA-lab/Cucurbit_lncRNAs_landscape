...
#SBATCH --output=Compare_RMfiltered.log					# Standard output and error log.
...
mkdir -p $WD1/03-Comparison_Genes_LncRNAs_RMfiltered
...
for spe in $Species_list; do
	srun -N1 -n1 -c2 --quiet --exclusive $F task_intersect_RMfiltered $spe $WD1 $WD2 $WD3 &
done
wait
...
Get_percentage_of_masked_genome.py \
	--path $WD1/01-Repeat_calling/06-RepeatMasker_RMfiltered \
...
for flag in $flags; do
	for confidence in $confidences; do
		 srun -N1 -n1 -c$SLURM_CPUS_PER_TASK --quiet --exclusive $F task_Figures_and_tables $WD $WD1 "03-Comparison_Genes_LncRNAs_RMfiltered" "06-RepeatMasker_RMfiltered" $flag $confidence $AS &
	done
	wait
done
