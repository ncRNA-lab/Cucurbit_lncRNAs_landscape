...
#SBATCH --job-name=Masklmch					# Job name.
#SBATCH --output=Repeat_Masker_mch.log			# Standard output and error log.
...
mkdir -p $WD1/01-Repeat_calling/05-RepeatMasker
mkdir -p $WD1/01-Repeat_calling/05-RepeatMasker/Logs
...
for spe in $Species_list; do
	srun -N1 -n1 -c16 --quiet --exclusive $F task_RepeatMasker $spe $AI $WD1 16 &
done
wait
