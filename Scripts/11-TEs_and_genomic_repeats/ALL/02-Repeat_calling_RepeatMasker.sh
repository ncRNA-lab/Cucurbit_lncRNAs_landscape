...
#SBATCH --job-name=RepMask				# Job name.
...
mkdir -p $WD1/01-Repeat_calling/02-RepeatMasker
mkdir -p $WD1/01-Repeat_calling/02-RepeatMasker/Logs
...
for spe in $Species_list; do
	srun -N1 -n1 -c16 --quiet --exclusive $F task_RepeatMasker $spe $AI $WD1 4 &
done
wait
