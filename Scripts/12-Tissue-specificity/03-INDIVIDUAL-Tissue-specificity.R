################################################################################
#
# INDIVIDUAL: TISSUE SPECIFICITY STUDY: APPROACH 1 - STEP 3
#
...
for (spe in species) {
  ...
  # Directories.
  if (!dir.exists(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3"))){
    dir.create(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3"))
  }
  if (!dir.exists(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe))){
    dir.create(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe))
  }
  ...
  if (length(files) > 0) {
    ...
    for (SRA.Study in SRA.Studies) {
      ...
      ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe, "/", SRA.Study, "_mean-TAU-Violin.png"), height = 10, width = 10, dpi = 600)
      ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe, "/", SRA.Study, "_mean-TAU-Violin.pdf"), height = 10, width = 10, dpi = 600)
      ...
      ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe, "/", SRA.Study, "_mean-TAU-Violin_by_class_code.png"), height = 10, width = 20, dpi = 600)
      ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe, "/", SRA.Study, "_mean-TAU-Violin_by_class_code.pdf"), height = 10, width = 20, dpi = 600)
...
write.table(TAB_mean_TAU_mod, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/mean-TAU.tsv"), sep = "\t", quote = F, row.names = F, col.names = T)
...
ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/mean-TAU-Violin_by_class_code.png"), height = 20, width = 10, dpi = 600)
ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/mean-TAU-Violin_by_class_code.pdf"), height = 20, width = 10, dpi = 600)
...
ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/mean-TAU-Violin_by_class_code-jitter.png"), height = 20, width = 10, dpi = 600)
ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/mean-TAU-Violin_by_class_code-jitter.pdf"), height = 20, width = 10, dpi = 600)
...
for (spe in species) {
  ...
  if (length(files) > 0) {
    ...
    for (SRA.Study in SRA.Studies) {
      ...
      # Violin plot
      gg = ggplot(tab_median_TAU, aes(x = Confidence, y = TAU, fill = Confidence)) + 
        geom_violin(color = "grey", position = position_dodge(1)) +
        geom_jitter(width = 0.4, color = "#8c8f8f", size = 0.1, alpha = 0.3) +
        stat_summary(fun = mean, geom = "point", size = 2, color = "black") +
        geom_boxplot(width = 0.1, color = "black", alpha = 0.2, outlier.shape = NA, position = position_dodge(1)) +
      ...
      ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe, "/", SRA.Study, "_median-TAU-Violin.png"), height = 10, width = 10, dpi = 600)
      ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe, "/", SRA.Study, "_median-TAU-Violin.pdf"), height = 10, width = 10, dpi = 600)
      
      rm(list = c("gg"))
      
      # Violin plot by class code.
      tab_median_TAU_mod = tab_median_TAU[tab_median_TAU$Class_code != "pc", c("ID_transcript", "Confidence", "Class_code", "TAU")]
      for (c in c("u", "x", "i", "o/e")) {
        pc = tab_median_TAU[tab_median_TAU$Class_code == "pc", c("ID_transcript", "Confidence", "Class_code", "TAU")]
        pc[pc == "pc"] = c
        tab_median_TAU_mod = rbind(tab_median_TAU_mod, pc)
      }
      tab_median_TAU_mod$Class_code = ifelse(tab_median_TAU_mod$Class_code == "u", "Intergenic lncRNA (u)", 
                                             ifelse(tab_median_TAU_mod$Class_code == "x", "Antisense lncRNA (x)",
                                                    ifelse(tab_median_TAU_mod$Class_code == "i", "Intronic lncRNA (i)",
                                                           ifelse(tab_median_TAU_mod$Class_code == "o/e", "Sense lncRNA (o/e)", tab_median_TAU_mod$Class_code))))
      tab_median_TAU_mod$Class_code = factor(tab_median_TAU_mod$Class_code, levels = c("Intergenic lncRNA (u)", "Antisense lncRNA (x)", "Intronic lncRNA (i)", "Sense lncRNA (o/e)"))
      tab_median_TAU_mod$Confidence = factor(tab_median_TAU_mod$Confidence, levels = c("Protein-coding", "Low-confidence lncRNA", "Medium-confidence lncRNA", "High-confidence lncRNA"))
      
      tab_median_TAU_mod$"SRA.Study" = SRA.Study
      tab_median_TAU_mod$"Spe" = spe
      tab_median_TAU_mod = tab_median_TAU_mod[,c("Spe", "SRA.Study", "ID_transcript", "Confidence", "Class_code", "TAU")]
      TAB_median_TAU_mod = rbind(TAB_median_TAU_mod, tab_median_TAU_mod)
      
      gg = ggplot(tab_median_TAU_mod, aes(x = Confidence, y = TAU, fill = Confidence)) + 
        geom_violin(color = "grey", position = position_dodge(1)) +
        geom_jitter(width = 0.4, color = "#8c8f8f", size = 0.1, alpha = 0.3) +
        stat_summary(fun = mean, geom = "point", size = 2, color = "black") +
        geom_boxplot(width = 0.1, color = "black", alpha = 0.2, outlier.shape = NA, position = position_dodge(1)) +
      ...
      ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe, "/", SRA.Study, "_median-TAU-Violin_by_class_code.png"), height = 10, width = 20, dpi = 600)
      ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/", spe, "/", SRA.Study, "_median-TAU-Violin_by_class_code.pdf"), height = 10, width = 20, dpi = 600)
      
      rm(list = c("gg", "c", "pc", "tab_median_TAU_mod"))
    }
  }
}
...
write.table(TAB_median_TAU_mod, paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/median-TAU.tsv"), sep = "\t", quote = F, row.names = F, col.names = T)
...
ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/median-TAU-Violin_by_class_code.png"), height = 20, width = 10, dpi = 600)
ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/median-TAU-Violin_by_class_code.pdf"), height = 20, width = 10, dpi = 600)
...
ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/median-TAU-Violin_by_class_code-jitter.png"), height = 20, width = 10, dpi = 600)
ggsave(paste0(path_tissue_specificity, "/approach_1/INDIVIDUAL/", flag, "/STEP3/median-TAU-Violin_by_class_code-jitter.pdf"), height = 20, width = 10, dpi = 600)

rm(list = c("gg", "TAB_median_TAU_mod"))
