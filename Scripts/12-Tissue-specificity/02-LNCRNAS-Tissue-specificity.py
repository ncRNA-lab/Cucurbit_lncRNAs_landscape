...
"""
LNCRNAS: TISSUE SPECIFICITY STUDY: APPROACH 1 - STEP 2

Calculation of metrics for assessing tissue specificity of transcripts (LncRNAs). 
...
def main():
    ...
    for spe in species:
        ...
        ## Files
        Files_median = [file for file in os.listdir(path_spe_in) if "_median.tsv" in file and "LNCRNAS-" in file]
        Files_mean = [file for file in os.listdir(path_spe_in) if "_mean.tsv" in file and "LNCRNAS-" in file]
        
        ## Median
        print("\t\tMedian values...")
        if len(Files_median) > 0:
            for file in Files_median:
                print("\t\t\tSRA.Study: " + file.replace("_median.tsv", "").replace("LNCRNAS-", ""))
        ...
        if len(Files_mean) > 0:
            for file in Files_mean:
                print("\t\t\tSRA.Study: " + file.replace("_mean.tsv", "").replace("LNCRNAS-", ""))
