################################################################################
# FIND HIGH-DENSITY REGIONS (TRANSCRIPT ENRICHED REGIONS)
################################################################################
...
###### 4.1.2  Create a table representing the gene density as the number of transcripts found in 100Kb size windows.
...
write.table(Tab_number_wide, paste0(WD, "/", spe, "-ALL-Number-100Kb.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)
...
write.table(Tab_number_summary_1, paste0(WD, "/", spe, "-ALL-Number-100Kb-HDWs-Collapse_1.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)

## Taking into account the class code and the chormosomes, draw a barplot with the number of high density regions.
gg = ggplot(Tab_number_summary_1, aes(x = Class_code, y = HDWs, fill = Class_code)) + 
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c( "#a2ded9", "#ece68f", "#decfa9", "#dcb8b8", "#b5cf9b")) +
  facet_grid(.~Chr) +
  xlab("") +
...
ggsave(paste0(WD, "/", spe, "-ALL-Number-100Kb-HDWs-BAR-1.png"), height = 5, width = 20, dpi = 600)

rm(list = c("gg"))

## Collapse the long table by class code.
Tab_number_summary_2 = Tab_number_long %>% group_by(Class_code, .drop = F) %>% summarise(HDWs = sum(HDW))

write.table(Tab_number_summary_2, paste0(WD, "/", spe, "-ALL-Number-100Kb-HDWs-Collapse_2.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)

## Taking into account the class code and the chormosomes, draw a barplot with the number of high density regions.
gg = ggplot(Tab_number_summary_2, aes(x = Class_code, y = HDWs, fill = Class_code)) + 
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("#a2ded9", "#ece68f", "#decfa9", "#dcb8b8", "#b5cf9b")) +
  xlab("") +
  ylab("Number of high density windows") +
...
ggsave(paste0(WD, "/", spe, "-ALL-Number-100Kb-HDWs-BAR-2.png"), height = 6, width = 7, dpi = 600)

rm(list = c("gg"))

###### 4.1.5 Draw venn diagram
...
write.table(Tab_number_wide_mod, paste0(WD, "/", spe, "-ALL-Number-100Kb-HDWs.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)

Tab_number_wide_mod_1 = as.data.frame(Tab_number_wide_mod[Tab_number_wide_mod$HDWs.Coincidences >= 1,])

## Create HDWs lists by class codes.
x = list(
...
###### 4.1.6 Percentage of high-density regions in lncRNAs (u, x, i, o/e) which coincide with high-density regions of genes (pc). 

TEMP = Tab_number_wide_mod_1[,c("Chr", "Start", "End", "ID", "HDW.pc", "HDW.u", "HDW.x", "HDW.i", "HDW.o/e")]
TEMP = melt(setDT(TEMP), id.vars = c("Chr", "Start", "End", "ID"), variable.name = "Class_code", value.name = "Value")
TEMP$Class_code = gsub("HDW.", "", TEMP$Class_code)
TEMP = as.data.frame(TEMP)

## Taking into account chromosomes.
RES1 = data.frame()
for (chr in chrs.sizes$Chr) {
  for (cl in c("u", "x", "i", "o/e")) {
    A = as.character(TEMP[TEMP$Chr == chr & TEMP$Class_code == cl & TEMP$Value == 1, "ID"])
    B = as.character(TEMP[TEMP$Chr == chr & TEMP$Class_code == "pc" & TEMP$Value == 1, "ID"])
    HDWs_coincidences = length(intersect(A, B))
    Total.HDWs_cl = sum(TEMP[TEMP$Chr == chr & TEMP$Class_code == cl, "Value"])
    Total.HDWs_pc = sum(TEMP[TEMP$Chr == chr & TEMP$Class_code == "pc", "Value"])
    res = data.frame(Class_code = cl, 
                     HDWs_coincidences = HDWs_coincidences, 
                     Total.HDWs_cl = Total.HDWs_cl, 
                     Total.HDWs_pc = Total.HDWs_pc)
    res$"Perc.Coincidence.HDWs_cl" = round((res$HDWs_coincidences*100)/Total.HDWs_cl, 2)
    res$"Perc.Coincidence.HDWs_pc" = round((res$HDWs_coincidences*100)/Total.HDWs_pc, 2)
    RES1 = rbind(RES1, res)
  }
}

write.table(RES1, paste0(WD, "/", spe, "-ALL-Number-100Kb-HDWs-Percentage_pc_overlap-1.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)

## Without taking into account chromosomes.
RES2 = data.frame()
for (cl in c("u", "x", "i", "o/e")) {
  A = as.character(TEMP[TEMP$Class_code == cl & TEMP$Value == 1, "ID"])
  B = as.character(TEMP[TEMP$Class_code == "pc" & TEMP$Value == 1, "ID"])
  HDWs_coincidences = length(intersect(A, B))
  Total.HDWs_cl = sum(TEMP[TEMP$Class_code == cl, "Value"])
  Total.HDWs_pc = sum(TEMP[TEMP$Class_code == "pc", "Value"])
  res = data.frame(Class_code = cl, 
                   HDWs_coincidences = HDWs_coincidences, 
                   Total.HDWs_cl = Total.HDWs_cl, 
                   Total.HDWs_pc = Total.HDWs_pc)
...
write.table(RES2, paste0(WD, "/", spe, "-ALL-Number-100Kb-HDWs-Percentage_pc_overlap-2.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)

rm(list = c("TEMP", "cl", "chr", "res", "A", "B", "HDWs_coincidences", "Total.HDWs_cl", "Total.HDWs_pc"))

rm(list = c("RES1", "RES2", "Tab_number_long", "Tab_number_summary_1", "Tab_number_summary_2", 
            "Tab_number_threshold", "Tab_number_wide", "Tab_number_wide_mod", "Tab_number_wide_mod_1"))
...
for (confidence in c("Low", "Medium", "High")) {
  ...
  ###### 4.2.2  Create a table with the number of transcripts by 100Kb window.
  ...
  write.table(Tab_number_summary_1, paste0(WD, "/", spe, "-", confidence, "-Number-100Kb-HDWs-Collapse_1.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)
  ...
  ggsave(paste0(WD, "/", spe, "-", confidence, "-Number-100Kb-HDWs-BAR-1.png"), height = 5, width = 20, dpi = 600)
  ...
  write.table(Tab_number_summary_2, paste0(WD, "/", spe, "-", confidence, "-Number-100Kb-HDWs-Collapse_2.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)
  ...
  ggsave(paste0(WD, "/", spe, "-", confidence, "-Number-100Kb-HDWs-BAR-2.png"), height = 5, width = 5, dpi = 600)
  ...
  png(filename = paste0(WD, "/", spe, "-", confidence, "-Number-100Kb-HDWs-VENN.png"), height = 5000, width = 5000, res=600)
  my.vennDiagram(Z,
                 include = "both",
                 names = NULL,
                 mar = rep(1,4),
  ...
  ###### 4.2.6 Percentage of high-density regions in lncRNAs (u, x, i, o/e) which coincide with high-density regions of genes (pc). 
  ...
  write.table(RES1, paste0(WD, "/", spe, "-", confidence, "-Number-100Kb-HDWs-Percentage_pc_overlap-1.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)
  ...
  write.table(RES2, paste0(WD, "/", spe, "-", confidence, "-Number-100Kb-HDWs-Percentage_pc_overlap-2.tsv"), col.names = T, sep = "\t", row.names = F, quote = F)
